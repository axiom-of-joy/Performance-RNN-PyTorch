...
def get_options():
    ...
    parser.add_option('-D', '--summary',
                      dest='summary',
                      type='choice',
                      choices=SUMMARY_CHOICES)

    # This one I'm not so sure about?
    # The original version in the docs also had action='store'.
    parser.add_option('-C', '--compress',
                      dest='compress',
                      type='string',
                      default='')  # Not sure what the default should be here?
    # Distiller End
...
# Distiller Begin
# The following learning rate scheduler was included in the Distiller example.
...
# Handle summary.
if options.summary:
    # distiller.model_summary can take an additional argument 'dataset', not sure what that does yet.
    distiller.model_summary(model, options.summary) 
    exit(0)

# Create message logger.
msglogger = apputils.config_pylogger('logging.conf', None)
# The following was included in the docs, but I think this implementation already has TensorBoard logging.
#tflogger = TensorBoardLogger(msglogger.logdir)
#tflogger.log_gradients = True
pylogger = PythonLogger(msglogger)

# Create a CompressionScheduler and configure it from YAML schedule file.
if options.compress:
    source = options.compress
    compression_scheduler = distiller.CompressionScheduler(model)
    # distiller.config.file_config takes an additional optional argument 'resumed_epoch'.
    # compression_scheduler is an optional argument.
    distiller.config.file_config(model, optimizer, options.compress, compression_scheduler)
# Distiller End
